...
export default function CartEdit({
	...
	const totalEditValor =
		editPedido?.produtos?.reduce(
	...
	return (
		...
		>
			...
			<div className="flex flex-col items-center">
				...
				<h3 className="my-3 font-semibold text-slate-500 mx-2">
					Data: {new Date(editPedido?.data).toLocaleDateString("pt-BR")}
				</h3>
			</div>
			{/* ----------------------- LISTA ----------------------- */}
			<div className="overflow-auto   grow  mb-12">
				{
					<div>
						{editPedido?.produtos?.map((item, index) => (
							<CartItemEdit key={item._id} index={index} data={item} />
						)) || []}
					</div>
					// )
				}
			...
			<div>
				...
				<div className=" mx-2">
					...
					<div className="flex  items-center  gap-4 my-4">
						<button
							onClick={
								editPedido?.produtos?.length > 0 ? saveNotify : undefined
							}
							className={`bg-slate-700 flex gap-2 text-white font-semibold py-4 px-6 rounded-lg ${
								editPedido?.produtos?.length > 0 && !hasAcerto
									? "hover:bg-slate-800  invisible"
									: "bg-cyan-800"
							}`}
						...
						<button
							onClick={
								editPedido?.produtos?.length > 0 ? editNotify : undefined
							}
							className={`bg-yellow-500 flex gap-2 text-white font-semibold py-4 px-6 rounded-lg ${
								editPedido?.produtos?.length > 0 && !hasAcerto
									? "hover:bg-yellow-600"
									: "invisible opacity-20 cursor-not-allowed"
							}`}
							disabled={hasAcerto || editPedido?.produtos?.length === 0}
						>
						...
						<Link to="/cartprintedit">
							<button className="border bg-black flex gap-2 text-white font-semibold py-4 px-6 rounded-lg hover:bg-white hover:text-black  border-black">
