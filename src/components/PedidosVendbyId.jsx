...
const PedidosVendbyId = () => {
	...
	const selectedPedido =
		acertos?.find((pedido) => pedido?.pedidoId === selectedPedId) || null;
	...
	const handleEditarObs = (e, key) => {
    setEditPedido({...editPedido.obs.obs, [key]: e.target.value})
  }
	...
	return (
		<div className="flex flex-col">
			<div>
				<label className="text-gray-800 font-bold text-lg" htmlFor="vendedora">
					Vendedora
				</label>
				<input
					type="text"
			...
			<p className="font-bold my-3 text-red-500">
				...
				{loading ? (
					...
					currentItems
						.filter((pdt) =>
							busca.toLowerCase() === ""
								? false
								: pdt.nome.toLowerCase().includes(busca.toLowerCase()),
						)
